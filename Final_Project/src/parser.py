...
def p_ComplementExpression(p):
    ...
    CreateTAC("~",newPlace+ST.currentscope,name+p[2]['scope']+end,None)
    Derivations.append(p.slice)     
    ...
    if( p[-4] == 'writeln'):
        for i in reversed(param_list):
            if i[1]=="INT":
                ...
                CreateTAC( "print_int", name+ST.checkscope(name)+end, None, None )
        param_list=[]
        ...
        for i in reversed(param_list):
            if i[1]=="INT":
                ...
                CreateTAC( "input_int", name+ST.checkscope(name)+end, None, None )
        param_list=[]
    ...
    if len(p)==5:
        ...
        if scope:
            ...
            if type(p[3])==type({}):
                ...
                name = p[3]
                if not p[3].isdigit():
                    if ST.checkscope(p[3]):
                        name = p[3]+ST.checkscope(p[3])
                    else:
                        print p[3]," is not defined "
                        sys.exit(0)
...
def p_Dowhile_M2(p):
    '''
        Dowhile_M2 : empty
    '''
    CreateTAC( "ifgoto_eq", p[-6][0], p[-2]['place']+p[-2]['scope'], 1)
...
def p_switch_M1(p):
    ...
    global s_cond
    global s_label
    if 'scope' in p[-2].keys():
        s_cond = p[-2]['place'] + p[-2]['scope']
    # else:
    #     s_cond = p[-2]['place']
...
def p_c_m1(p):
    ...
    label = ST.get_label()
    if 'scope' in p[-2].keys():
        CreateTAC( "ifgoto_neq", label , s_cond, p[-2]['place']+p[-2]['scope']  )
    elif p[-2].isdigit() or p[-2].ischar():
...
def p_ReturnStatement(p):
    ...
    if not name.isdigit():
        if 'scope' in p[2].keys():
            ...
            if "[" in name:
                name = p[2]['place'].strip("[")[0]
                end = p[2]['place'].strip("[")[1]
            if "." in name:
                name = p[2]['place'].strip(".")[0]
                end = p[2]['place'].strip(".")[1]
            name = name + ST.checkscope(name)+end
    CreateTAC("=",FuncLabel+".ret",name,None)    
    if FuncLabel=='main':
        CreateTAC( 'exit',name,None, None )    
    else:
        CreateTAC( 'ret', name, None, None )    
    Derivations.append(p.slice)
